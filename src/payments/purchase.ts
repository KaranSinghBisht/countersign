...
import { AgentProposalSchema, accept, type Cart } from "../gate/accept.js";
import { claim, complete } from "../http/idempotency.js";
...
import { CURRENCIES, type Money, money, zero } from "../money/money.js";
import { ENGINE_VERSION, type SpendState } from "../policy/engine.js";
...
export const CartSchema = z
  .object({
    ...
    payee: z.object({ id: z.string().min(1) }).strict(),
    rail: z.string().min(1),
...
export const PurchaseBodySchema = z
  .object({
    ...
    open_jws: z.string().min(1),
    closed_jws: z.string().min(1),
    cart: CartSchema,
...
export const NonceBodySchema = z.object({ issued_to: z.string().min(1) }).strict();
...
export async function purchase(sql: Sql, input: PurchaseInput): Promise<PurchaseResponse> {
  ...
  try {
    ...
    try {
      await complete(sql, input.actorId, input.idempotencyKey, 500, {
        outcome: "error",
        detail: "internal",
      });
    } catch {
...
async function execute(sql: Sql, input: PurchaseInput): Promise<ExecutedResponse> {
  ...
  return withTxn(sql, async (tx) => {
    ...
    const spent = await spendSnapshot(tx, chain.open.jti, input.cart.currency);
    const gated = accept(input.cart, input.proposal, chain.constraints, spent, input.now);
    ...
    if (result.outcome === "already_in_flight") {
      return respond(tx, input, 409, { outcome: "already_in_flight", open_jti: result.openJti });
    }
...
async function spendSnapshot(
  ...
  const rows = await sql<{ spent_minor: bigint; actions: number }[]>`
		SELECT spent_minor, actions FROM mandate_spend WHERE open_jti = ${openJti}
	`;
  const row = rows[0];
  const spent: Money = row === undefined ? zero(currency) : money(row.spent_minor, currency);
  return {
    spent,
    actions: row === undefined ? 0 : row.actions,
    recent: [],
  };
}
